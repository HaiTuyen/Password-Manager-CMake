...
string PasswordManager::get_master_password() const
{
	return master_password;
}

string PasswordManager::get_master_username() const
{
	return this->master_username;
}

void PasswordManager::set_master_password(const string& master_password)
{
	this->master_password = master_password;
}

void PasswordManager::set_master_user(const string& master_username)
{
	this->master_username = master_username;
}

string PasswordManager::encrypt_password(string password)
{
	AESEncryption aes(this->master_password, password);
	return aes.encrypt();
}

string PasswordManager::decrypt_password(string cipher_hex_str)
{
	string cipher_byte_str = convert_hex_str_to_byte_str(cipher_hex_str);
	AESDecryption aes(this->master_password, cipher_byte_str);
	return aes.decrypt();
}


bool PasswordManager::verify_user(string user)
{
	string master_password_from_db = db.query_master_password_of_user(user);
...
void PasswordManager::create_user(string username)
{
	db.create_user(username, this->master_password);
}

vector<Account> PasswordManager::find_all_credentials()
{
	vector<Account> accounts = db.query_all_credentials(this->master_username);
	return accounts;
}

vector<Account> PasswordManager::find_credentials_of_service(string service)
{
	vector<Account> accounts = db.query_credentials(this->master_username, service);
	return accounts;
}

vector<Account> PasswordManager::find_one_credential_of_service_and_username(string service, string username)
{
	return db.query_credential(this->master_username, service, username);
}

void PasswordManager::add_credential(string service, string username, string password, string description)
{
	string encrypted_password = convert_byte_str_to_hex_str(encrypt_password(password));
	db.add_credential(this->master_username, service, username, encrypted_password, description);
}

void PasswordManager::edit_credential(string service, string username, string new_password)
{
	db.edit_credential(this->master_username, service, username, convert_byte_str_to_hex_str(encrypt_password(new_password)));
}

void PasswordManager::remove_credential(string service, string username)
{
...
void PasswordManager::change_master_password(string new_master_password)
{
	try
	{
		...
		for (Account account : accounts)
		{
			...
			string decrypted_password = aes_decrypt.decrypt();
			//cout << "Decrypted password: " << decrypted_password << endl;
			AESEncryption aes_encrypt(new_master_password, decrypted_password);
			string new_encrypted_password = convert_byte_str_to_hex_str(aes_encrypt.encrypt());
...
string PasswordManager::generate_random_password()
{
